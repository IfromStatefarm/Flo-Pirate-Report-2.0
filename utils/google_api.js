...
export async function getEventData(vertical) {
  ...
  const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
  const data = await res.json();
  if (!data.values || data.values.length < 1) return { searchUrl: null, eventMap: {} };
...
export async function ensureDailyScreenshotFolder(token, dateStr) {
  ...
  return await findOrCreateFolder(token, masterScreenshotFolderId, dateStr);
}
...
export async function fetchConfig() {
  ...
  const data = await res.json();
  if (!data.files || data.files.length === 0) throw new Error("Config file not found. Folder appears empty or inaccessible.");
