...
export function createRumbleWorkflow({
  ...
  async function advance(currentUrl, senderTabId) {
    ...
    const senderTab = senderTabId ? await chrome.tabs.get(senderTabId).catch(() => null) : null;
    const reportHandler = handleRumbleBatchReport || handleBatchReport;
    const response = await reportHandler(session.formData, {
      composerTabId: senderTabId,
